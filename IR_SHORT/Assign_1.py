...
# This converts the sparse matrix into a dense matrix (array) since np.dot() and np.linalg.norm() work better with dense matrices.

# [0] to extract the first row of the dense matrix since we are only interested in the vector corresponding to the single document (doc1, doc2, doc3).  
